...
En este trabajo analizamos el uso de modelos lineales y algoritmos de optimización aplicados a un conjunto de datos de propiedades inmobiliarias del barrio de Boedo, en la Ciudad de Buenos Aires. Se busca entender cómo variables como la superficie cubierta (Sup) y el fondo (Fondo) influyen en el precio (Precio) de venta de las casas. Utilizaremos métodos de optimización como Luus-Jaakola y gradiente descendente para ajustar un modelo lineal a los datos.
...
# Inicialización
set.seed(123)
mu <- runif(1, min = min(data$Precio), max = max(data$Precio))
alpha <- runif(1, min = -1000, max = 1000)
best_loss <- Inf
delta <- c(1e5, 1e3)  # Tamaño inicial de la región de búsqueda
n_iter <- 1000
...
for (i in 1:n_iter) {
  ...
  mu_candidate <- mu + runif(1, -delta[1], delta[1])
  alpha_candidate <- alpha + runif(1, -delta[2], delta[2])
  
  # Calcular pérdida
  preds <- mu_candidate + alpha_candidate * data$Sup
  loss <- mean((data$Precio - preds)^2)
...
plot_ly(df_params, x = ~Iteración) %>%
  add_trace(y = ~Mu, name = 'Mu', type = 'scatter', mode = 'lines') %>%
  add_trace(y = ~Alpha, name = 'Alpha', type = 'scatter', mode = 'lines', yaxis = "y2") %>%
  layout(title = 'Evolución de los Parámetros con Luus-Jaakola',
         yaxis = list(title = 'Mu'),
         yaxis2 = list(title = 'Alpha', overlaying = 'y', side = 'right'),
         xaxis = list(title = 'Iteración'))
```
...
Existen diferentes variantes del gradiente descendente:
* **Gradiente Descendente por Lotes (Batch Gradient Descent)**: Utiliza todo el conjunto de datos para calcular el gradiente en cada paso. Es estable pero puede ser lento en conjuntos de datos grandes.
...
```{r}
# Inicialización
mu_gd <- runif(1, min = min(data$Precio), max = max(data$Precio))
alpha_gd <- runif(1, min = -1000, max = 1000)
learning_rate <- 1e-7
n_iter_gd <- 1000
...
for (i in 1:n_iter_gd) {
  # Predicciones
  preds <- mu_gd + alpha_gd * data$Sup
  errors <- preds - data$Precio
  
  # Calcular gradientes
  mu_grad <- (2 / nrow(data)) * sum(errors)
  alpha_grad <- (2 / nrow(data)) * sum(errors * data$Sup)
...
Luego, graficamos las líneas de nivel (contornos) de la función de pérdida y superponemos la trayectoria del gradiente descendente:

```{r}
# Graficar las líneas de nivel de la función de pérdida
contour_plot <- plot_ly(x = mu_seq, y = alpha_seq, z = loss_grid, type = 'contour',
                        colorscale = 'Viridis') %>%
  layout(title = 'Contorno de la Función de Pérdida',
         xaxis = list(title = 'Mu'),
         yaxis = list(title = 'Alpha'))

# Añadir la trayectoria del gradiente descendente
trajectory <- data.frame(Mu = mu_history_gd, Alpha = alpha_history_gd, Loss = loss_history_gd)
contour_plot <- contour_plot %>%
  add_trace(data = trajectory, x = ~Mu, y = ~Alpha, type = 'scatter', mode = 'lines+markers',
            line = list(color = 'red'), marker = list(size = 4, color = 'red'), name = 'Trayectoria GD')

# Marcar el mínimo encontrado
min_point <- data.frame(Mu = mu_history_gd[n_iter_gd], Alpha = alpha_history_gd[n_iter_gd])
contour_plot <- contour_plot %>%
  add_trace(data = min_point, x = ~Mu, y = ~Alpha, type = 'scatter', mode = 'markers',
            marker = list(size = 8, color = 'green'), name = 'Mínimo Encontrado')

contour_plot
```

En este gráfico, la función de pérdida se representa mediante líneas de nivel, con $\mu$ en el eje X y $\alpha$ en el eje Y. Las áreas más claras indican valores de pérdida menores. La trayectoria del gradiente descendente se muestra con puntos y líneas rojas, y el mínimo encontrado está marcado con un punto verde.
...
cat("Valores obtenidos con lm():\n")
cat("mu =", round(mu_lm, 2), "\n")
cat("alpha =", round(alpha_lm, 2), "\n")
```

#### Comparación de coeficientes
Ahora, comparamos los coeficientes obtenidos con los diferentes métodos:
...
En el gráfico anterior, las líneas representan las predicciones de cada método:

* Luus-Jaakola: Línea roja continua.
* Gradiente Descendente: Línea verde continua.
* lm(): Línea azul discontinua.

Podemos observar que las tres líneas prácticamente se superponen, indicando que todos los métodos proporcionan un ajuste muy similar.

#### Evaluación de los Modelos
Para cuantificar el rendimiento de cada modelo, calculamos el error cuadrático medio (MSE) de las predicciones:
...
# Mostrar los MSE
cat("Error Cuadrático Medio (MSE) de cada método:\n")
cat("Luus-Jaakola:", round(mse_lj, 2), "\n")
cat("Gradiente Descendente:", round(mse_gd, 2), "\n")
cat("lm():", round(mse_lm, 2), "\n")

```
Los valores de MSE obtenidos son muy similares, confirmando que todos los métodos logran un ajuste comparable.

## Conclusiones
En este trabajo, hemos aplicado modelos lineales y algoritmos de optimización para predecir el precio de venta de casas en el barrio de Boedo, en base a la superficie cubierta. Utilizamos dos métodos de optimización: Luus-Jaakola y gradiente descendente. Ambos métodos convergieron a soluciones similares, confirmando la efectividad de estos algoritmos en la optimización de funciones convexas.
